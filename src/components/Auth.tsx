...
export const Auth = () => {
  ...
  const handleAuth = async (e: React.FormEvent) => {
    ...
    // Standardizing to @cc.com - NEVER CHANGE THIS AGAIN
      ...
      <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="bg-[#1a0505] p-10 rounded-[3rem] border border-white/5 w-full max-w-md shadow-2xl text-center">
        ...
        <form onSubmit={handleAuth} className="space-y-4">
          <input 
            ...
            className="w-full bg-black p-5 rounded-2xl outline-none border border-white/10 focus:border-red-600 font-bold uppercase text-white transition-all" 
            value={username} 
          ...
          <input 
            ...
            className="w-full bg-black p-5 rounded-2xl outline-none border border-white/10 focus:border-red-600 font-bold text-white transition-all" 
            value={password} 
