...
function App() {
  const { user, balance, isOwner, signOut } = useWallet();
  ...
  return (
    <div className="min-h-screen bg-[#0f0202] text-white selection:bg-red-500/30">
      {/* Navbar */}
        ...
        <div className="flex items-center gap-4">
          ...
          <div className="relative">
            <motion.div 
              ...
              className={`w-10 h-10 rounded-full border flex items-center justify-center cursor-pointer ${isOwner ? 'bg-red-600 border-white shadow-lg' : 'bg-[#2a0a0a] border-white/10'}`}
            >
            ...
            <AnimatePresence>
              {showMenu && (
                <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 10 }} className="absolute right-0 mt-4 w-60 bg-[#1a0505] border border-white/10 rounded-2xl shadow-2xl overflow-hidden p-2 z-[100]">
                  ...
                  <button onClick={() => signOut()} className="w-full flex items-center gap-3 px-4 py-3 rounded-xl text-xs font-black hover:bg-red-600 text-red-500 hover:text-white uppercase">
      ...
      <main className="max-w-6xl mx-auto p-6 lg:p-12">
        <AnimatePresence mode="wait">
          {activeView === 'home' ? (
            <motion.div key="home" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-12">
              <div className="relative overflow-hidden w-full h-80 bg-gradient-to-br from-red-600 to-rose-800 rounded-[3rem] p-12 flex flex-col justify-center shadow-2xl">
                <h1 className="text-6xl font-black mb-2 uppercase italic tracking-tighter leading-none">Sweet Wins <br/>Await You</h1>
...
function GameCard({ title, color, onClick, darkText = false }: any) {
  return (
    <motion.div whileHover={{ scale: 1.05, y: -10 }} whileTap={{ scale: 0.98 }} onClick={onClick} className={`h-64 rounded-[2.5rem] bg-gradient-to-br ${color} p-8 cursor-pointer overflow-hidden shadow-2xl border-2 border-white/5 flex flex-col justify-end`}>
