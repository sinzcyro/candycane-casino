      ...
      <nav className="border-b border-white/5 bg-[#1a0505]/95 backdrop-blur-md px-6 py-2 flex items-center justify-between sticky top-0 z-50">
        ...
        <div className="flex items-center gap-4">
          ...
          <div className="relative">
            <div onClick={() => setShowMenu(!showMenu)} className={`w-10 h-10 rounded-full border flex items-center justify-center cursor-pointer ${isOwner ? 'bg-red-600 border-white shadow-lg' : 'bg-[#2a0a0a] border-white/10'}`}>
              {isOwner ? <Crown size={20} /> : <UserIcon size={20} />}
            </div>
            <AnimatePresence>
              {showMenu && (
                <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="absolute right-0 mt-4 w-64 bg-[#1a0505] border border-white/10 rounded-2xl shadow-2xl p-2 z-[100]">
                  ...
                  <button onClick={() => signOut()} className="w-full flex items-center gap-3 px-4 py-3 text-red-500 font-black uppercase text-xs"> <LogOut size={16}/> Logout</button>
                </motion.div>
              )}
            </AnimatePresence>
          </div>
