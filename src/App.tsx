              ...
              {showMenu && (
                <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 10 }} className="absolute right-0 mt-4 w-64 bg-[#1a0505] border border-white/10 rounded-2xl shadow-2xl overflow-hidden p-2 z-[100]">
                  <div className="px-4 py-4 border-b border-white/5 mb-2 text-center font-black text-red-500 uppercase italic truncate text-lg">
                    {user.username} {isOwner && <Crown size={14} />}
                  </div>
                  ...
                  <MenuBtn icon={<Info size={16}/>} label="ABOUT US" onClick={() => {setActiveView('about'); setShowMenu(false)}} />
              ...
              <div className="lg:col-span-8 space-y-8">
                ...
                <div onClick={() => setActiveView('explore')} className="bg-[#1a0505] border border-red-500/20 p-6 rounded-[2.5rem] flex items-center justify-between shadow-xl cursor-pointer hover:border-red-500 transition-all group">
                    <div className="flex items-center gap-4">
                        ...
                        <div><h2 className="text-xl font-black italic uppercase">Go Exploring</h2><p className="text-white/40 text-xs font-bold uppercase">Find free items in the wild</p></div>
                    </div>
...
function GameCard({ title, icon, color, onClick, darkText = false }: any) {
  return (
    <motion.div 
      ...
      whileTap={{ scale: 0.98 }} 
      onClick={onClick} 
      className={`h-44 rounded-[2.5rem] bg-gradient-to-br ${color} p-8 cursor-pointer overflow-hidden shadow-2xl border border-white/5 flex flex-col justify-between group transition-all`}
    >
