...
type ViewType = 'home' | 'coinflip' | 'mines' | 'blackjack' | 'crash' | 'slots' | 'roulette' | 'cups' | 'tower' | 'race' | 'plinko' | 'leaderboard' | 'admin';
              ...
              {showMenu && (
                <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 10 }} className="absolute right-0 mt-4 w-64 bg-[#1a0505] border border-white/10 rounded-2xl shadow-2xl overflow-hidden p-2 z-[100]">
                  <div className="px-4 py-4 border-b border-white/5 mb-2 bg-white/5 rounded-2xl text-center">
                    <p className="font-black text-red-500 flex items-center justify-center gap-2 uppercase italic truncate text-lg">{user.username} {isOwner && <Crown size={14} />}</p>
                  </div>
        ...
        <AnimatePresence mode="wait">
          {activeView === 'home' ? (
            <motion.div key="home" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-10 py-6">
              <div className="relative overflow-hidden w-full h-72 bg-gradient-to-br from-red-600 to-rose-900 rounded-[3rem] p-12 flex flex-col justify-center shadow-2xl">
