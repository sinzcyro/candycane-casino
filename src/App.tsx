      ...
      <nav className="border-b border-white/5 bg-[#1a0505]/95 backdrop-blur-md px-6 py-2 flex items-center justify-between sticky top-0 z-50">
        ...
        <div className="flex items-center gap-4">
          <motion.div 
            ...
            className={`bg-[#2a0a0a] border border-red-500/20 px-4 py-2 rounded-xl flex items-center gap-3 shadow-lg transition-all ${hasGlow ? 'shadow-[0_0_20px_rgba(239,68,68,0.4)] border-red-500' : ''}`}
          >
              ...
              {showMenu && (
                <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 10 }} className="absolute right-0 mt-4 w-60 bg-[#1a0505] border border-white/10 rounded-2xl shadow-2xl overflow-hidden p-2 z-[100]">
                  <div className="px-4 py-3 border-b border-white/5 mb-2">
                    <p className="text-[10px] font-black text-white/30 uppercase tracking-widest leading-none mb-1">User</p>
                    <p className="font-black text-red-500 flex items-center gap-2 uppercase italic">{user.username} {isOwner && <Crown size={12} />}</p>
                  </div>
...
const MenuBtn = ({ icon, label, onClick, danger = false }: any) => (
  <button onClick={onClick} className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl text-xs font-black transition-colors ${danger ? 'text-red-500 hover:bg-red-500 hover:text-white' : 'hover:bg-white/5 text-white/70 hover:text-white'}`}>{icon} {label}</button>
);
...
function GameCard({ title, desc, color, onClick, darkText = false }: any) {
  return (
    <motion.div whileHover={{ scale: 1.03, y: -5 }} whileTap={{ scale: 0.98 }} onClick={onClick} className={`h-64 rounded-[2.5rem] bg-gradient-to-br ${color} p-8 cursor-pointer overflow-hidden shadow-xl border-2 border-white/5 relative group`}>
      <div className="flex items-center gap-2 mb-2 text-red-500/50"><Candy size={18} /><span className="text-[10px] font-black uppercase tracking-widest">Candy Original</span></div>
